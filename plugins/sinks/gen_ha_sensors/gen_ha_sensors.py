...
def env_vars(config):
    ...
    if os.environ.get("GENHASENSORS_GENERATEFREQ"):
        config["generator"]["generate_freq"] = os.environ.get(
            "GENHASENSORS_GENERATEFREQ", "10"
        )
    if os.environ.get("GENHASENSORS_FILEPREFIX"):
        config["generator"]["filename_prefix"] = os.environ.get(
            "GENHASENSORS_FILEPREFIX", ""
        )
    ...
    while not do_stop():
        ...
        if i % often == 0:
            ...
            for part in unique_parts:
                ...
                filtered_items = {
                    k: v for k, v in sma_items.items() if k.split(".")[0] == part
                }
